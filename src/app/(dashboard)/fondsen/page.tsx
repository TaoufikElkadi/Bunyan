...
import { Plus, Landmark, TrendingUp, ArrowUpRight, ArrowDownRight } from 'lucide-react'
...
export default async function FondsenPage() {
  ...
  const prevMonthTotal = (prevMonthDonations ?? []).reduce((sum, d) => sum + d.amount, 0)
  ...
  funds?.forEach((f) => { fundMap[f.id] = f.name })
  ...
  const fundNames = [...new Set(Object.values(monthlyAgg).flatMap((m) => Object.keys(m)))]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[26px] font-bold tracking-[-0.5px] text-[#261b07]">Fondsen</h1>
          <p className="text-[13px] text-[#a09888] mt-0.5">Beheer uw fondsen en bekijk de prestaties</p>
        </div>
        {canEdit && (
          <FundDialog
            mode="create"
      ...
      {funds && funds.length > 0 ? (
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-2xl bg-white p-5 shadow-[0_1px_3px_rgba(38,27,7,0.04),0_1px_2px_rgba(38,27,7,0.02)] sm:col-span-2">
              ...
              <p className="text-[12px] text-[#b5b0a5] mt-2">over alle fondsen · alle tijd</p>
            </div>
            ...
            <div className="rounded-2xl bg-white p-5 shadow-[0_1px_3px_rgba(38,27,7,0.04),0_1px_2px_rgba(38,27,7,0.02)]">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#C87D3A]/10">
                  <TrendingUp className="h-4 w-4 text-[#C87D3A]" strokeWidth={1.5} />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span
                  className={`inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${
                    trendUp ? 'bg-[#e8f0d4] text-[#4a7c10]' : 'bg-red-50 text-red-500'
                  }`}
                >
          ...
          {chartData.length > 0 && (
            <div className="rounded-2xl bg-white shadow-[0_1px_3px_rgba(38,27,7,0.04),0_1px_2px_rgba(38,27,7,0.02)]">
              <div className="px-6 pt-5 pb-1">
                <h3 className="text-[15px] font-semibold text-[#261b07] tracking-tight">Donaties per fonds</h3>
                <p className="text-[12px] text-[#a09888] mt-0.5">Maandelijks overzicht per fonds</p>
              </div>
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#e3dfd5] bg-white py-16">
          ...
          <p className="text-[14px] font-medium text-[#261b07]">Nog geen fondsen aangemaakt</p>
          <p className="text-[12px] text-[#b5b0a5] mt-1">Maak een fonds aan om donaties te ontvangen</p>
        </div>
      )}
    </div>
  )
