...
const selectClasses =
  "h-9 rounded-lg border border-[#e3dfd5] bg-white px-3 text-[13px] text-[#261b07] transition-all focus:border-[#261b07]/30 focus:outline-none focus:ring-1 focus:ring-[#261b07]/10 hover:border-[#d5cfb8]";
const inputClasses =
  "h-9 rounded-lg border border-[#e3dfd5] bg-white px-3 text-[13px] text-[#261b07] placeholder:text-[#b5b0a5] transition-all focus:border-[#261b07]/30 focus:outline-none focus:ring-1 focus:ring-[#261b07]/10 hover:border-[#d5cfb8]";
...
export default async function DonatiesPage({
  ...
  const donorJoin = searchQuery
    ? "donors!inner(name, email)"
    : "donors(name, email)";
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent border-[#e3dfd5]">
                    <TableHead className="h-10 px-6 text-[12px] font-medium text-[#a09888] uppercase tracking-wide">
                      Datum
                    </TableHead>
                    ...
                    <TableHead className="h-10 px-6 text-[12px] font-medium text-[#a09888] uppercase tracking-wide text-right">
                      Bedrag
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {filteredDonations.map(
                    (donation: Record<string, unknown>) => {
                      const status = STATUS_LABELS[
                        ...
                        label: donation.status as string,
                        variant: "secondary" as const,
                      };
                      const donors = donation.donors as {
                        name?: string;
                      } | null;
                      const fundRecord = donation.funds as {
                        name?: string;
                      } | null;
                      const statusColor =
                        STATUS_COLORS[donation.status as string] ??
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="px-4 py-4 text-[13px] text-[#8a8478] hidden md:table-cell">
                            {METHOD_LABELS[donation.method as string] ??
                              (donation.method as string)}
                          </TableCell>
                          <TableCell className="px-4 py-4">
                            <span
                              className={`inline-flex items-center rounded-md border px-2 py-0.5 text-[11px] font-medium ${statusColor}`}
                            >
                              {status.label}
                            </span>
                          </TableCell>
