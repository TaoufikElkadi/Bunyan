...
export default async function LedenPage({
  ...
  if (!limits.hasMemberIntelligence) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-[28px] font-bold tracking-[-0.5px] text-[#261b07]">Leden</h1>
        ...
        <div className="rounded-xl border border-[#e3dfd5] bg-white">
          <div className="flex flex-col items-center justify-center py-16 px-6">
            ...
            <p className="text-[13px] text-[#a09888] text-center max-w-sm leading-relaxed">
              Ledenintelligentie is beschikbaar vanaf het Starter-abonnement. Upgrade om inzichten in uw gemeenschap te krijgen.
            </p>
          </div>
        </div>
      </div>
  ...
  function filterUrl(overrides: Record<string, string>) {
    ...
    if (overrides.risk ?? riskFilter) p.set('risk', overrides.risk ?? riskFilter)
      ...
      <div className="rounded-xl border border-[#e3dfd5] bg-white overflow-hidden">
        ...
        {members.length > 0 ? (
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent border-[#e3dfd5]">
                  <TableHead className="h-10 px-4 sm:px-6 text-[12px] font-medium text-[#a09888] uppercase tracking-wide">Naam</TableHead>
                  <TableHead className="h-10 px-4 text-[12px] font-medium text-[#a09888] uppercase tracking-wide hidden sm:table-cell">Status</TableHead>
                  <TableHead className="h-10 px-4 text-[12px] font-medium text-[#a09888] uppercase tracking-wide hidden md:table-cell">Verloop</TableHead>
                  <TableHead className="h-10 px-4 text-[12px] font-medium text-[#a09888] uppercase tracking-wide text-right hidden sm:table-cell">Totaal</TableHead>
                  ...
                  <TableHead className="h-10 px-4 text-[12px] font-medium text-[#a09888] uppercase tracking-wide hidden lg:table-cell">Frequentie</TableHead>
              ...
              <TableBody>
                {members.map((member) => (
                  <TableRow key={member.id} className="hover:bg-[#fafaf8] border-[#e3dfd5]/60">
                    <TableCell className="px-4 sm:px-6 py-3 sm:py-4 text-[13px] font-medium">
                      ...
                      {member.email && (
                        <p className="text-[11px] text-[#a09888] mt-0.5 truncate max-w-[180px]">{member.email}</p>
                      )}
                    ...
                    <TableCell className="px-6 py-4 text-[13px] text-[#8a8478] hidden md:table-cell">
                      {member.last_donated_at
                        ? new Date(member.last_donated_at).toLocaleDateString('nl-NL', {
                            day: 'numeric',
                            month: 'short',
                            year: 'numeric',
                          })
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6">
            ...
            <h3 className="text-[13px] font-medium text-[#261b07] mb-1.5">Geen leden gevonden</h3>
