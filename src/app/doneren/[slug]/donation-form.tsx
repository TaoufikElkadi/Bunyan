...
export function DonationForm({
  ...
  const feeCents =
    amountCents > 0 ? calculateCoverFee(amountCents, "stripe") : 0;
      ...
      >
          ...
          <div>
            ...
            <div className="space-y-2">
              {funds.map((fund, index) => {
                ...
                const pct = hasGoal
                  ? Math.min((fund.raised / fund.goal_amount!) * 100, 100)
                  : null;
                return (
                  <button
                    key={fund.id}
                    ...
                    <div className="flex items-center gap-3.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {fund.description && (
                          <div
                            className="text-xs mt-0.5 line-clamp-1"
                            style={{ color: active ? "#8B9CC0" : "#9B8E7B" }}
                          >
                            {fund.description}
                          </div>
                        )}
        ...
        {!isRecurring && amountCents >= 100 && (
          ...
          >
            ...
            <HeartIcon
              className="size-4"
              style={{ color: coverFee ? accent : "#D4CFC5" }}
            />
          </label>
        )}
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            />
            {isRecurring && (
              <p className="text-xs mt-1.5 px-1" style={{ color: "#9B8E7B" }}>
      ...
      <div className="px-5 pt-5 pb-6 space-y-5">
        ...
        >
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex-1">
              <p className="font-semibold text-sm" style={{ color: "#1B2541" }}>
                {mosqueName}
              </p>
              {isRecurring && (
                <p className="text-xs" style={{ color: "#9B8E7B" }}>
                  {t(`donate.frequency.${frequency.replace("-", "_")}`)}
                </p>
              )}
            </div>
