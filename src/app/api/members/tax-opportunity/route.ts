...
const TAX_RATE = 0.3697 // IB Box 1, first bracket
...
export async function GET() {
  try {
    ...
    const totalAnnual = donors.reduce((sum, d) => sum + (d.estimated_annual ?? 0), 0)
    const avg_estimated_annual = eligible_count > 0 ? Math.round(totalAnnual / eligible_count) : 0
