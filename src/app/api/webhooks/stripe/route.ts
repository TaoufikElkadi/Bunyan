...
// Capped at ALERT_MAX_ENTRIES to prevent unbounded growth on long-lived instances.
// ---------------------------------------------------------------------------
const alertCooldowns = new Map<string, number>();
const ALERT_COOLDOWN_MS = 5 * 60 * 1000;
const ALERT_MAX_ENTRIES = 100;

function shouldSendAlert(eventType: string): boolean {
  const now = Date.now();

  // Purge expired entries and enforce cap
  if (alertCooldowns.size >= ALERT_MAX_ENTRIES) {
    for (const [key, ts] of alertCooldowns) {
      if (now - ts >= ALERT_COOLDOWN_MS) alertCooldowns.delete(key);
    }
    // If still over cap after expiry cleanup, drop oldest entries
    if (alertCooldowns.size >= ALERT_MAX_ENTRIES) {
      const sorted = [...alertCooldowns.entries()].sort((a, b) => a[1] - b[1]);
      const toDelete = sorted.slice(0, Math.ceil(sorted.length * 0.1));
      for (const [key] of toDelete) alertCooldowns.delete(key);
    }
  }
 ...
 */
async function verifyActiveMosque(
  admin: AdminClient,
  mosqueId: string,
): Promise<boolean> {
...
async function handlePaymentSuccess(
  ...
  // Optimistic locking: only update if still pending to prevent double-processing
  ...
  if (updateError && updateError.code !== "PGRST116") {
    throw new Error(
      `Failed to update donation ${donation.id}: ${updateError.message}`,
    );
  }

  if (!updated) return; // Already processed by a concurrent request
  ...
  // Fire-and-forget — don't block the webhook response
  void sendDonationConfirmation(admin, pi.id, pi.metadata).catch((err) =>
...
async function handleFirstSubscriptionPayment(
  ...
  const pi = (await stripe.paymentIntents.retrieve(
    paymentIntentId,
  )) as Stripe.PaymentIntent & { invoice?: string | { id: string } };
  ...
  if (!recurring) {
    ...
    console.error(
      `Webhook: subscription ${subscriptionId} not found in recurrings, falling back to PI metadata`,
    );

    if (pi.metadata?.mosque_id && pi.metadata?.fund_id) {
      // Verify mosque exists and is active before inserting from metadata
      const mosqueValid = await verifyActiveMosque(
        admin,
        pi.metadata.mosque_id,
      );
      if (!mosqueValid) {
        console.warn(
      ...
      const { error: fallbackError } = await admin.from("donations").insert({
      ...
      if (fallbackError) {
        throw new Error(
          `Failed to insert fallback recurring donation: ${fallbackError.message}`,
        );
      }
  ...
  if (insertError) {
    throw new Error(
      `Failed to insert first recurring donation: ${insertError.message}`,
    );
  }
  ...
  // Fire-and-forget — don't block the webhook response
  void sendRecurringPaymentConfirmation(admin, recurring).catch((err) =>
    console.error("Email error:", err),
  );
}
...
async function incrementPlanUsage(admin: AdminClient, mosqueId: string) {
  ...
  if (error) {
    console.error(
      `Failed to increment plan usage for mosque ${mosqueId}:`,
      error,
    );
  }
}

async function handlePaymentFailed(admin: AdminClient, pi: { id: string }) {
  const { data: donation } = await admin
    .from("donations")
    .select("id, status")
    .eq("stripe_payment_intent_id", pi.id)
  ...
  // Optimistic locking: only update if not already failed
  const { data: updated, error: updateError } = await admin
    .from("donations")
    .update({
      status: "failed",
...
async function handleInvoicePaymentSucceeded(
  ...
  if (!recurring) {
    console.error(
      `Webhook: subscription ${subscriptionId} not found in recurrings`,
    );
    return;
  }
  ...
  if (insertError) {
    throw new Error(
      `Failed to insert recurring donation: ${insertError.message}`,
    );
  }
  ...
  if (recurringUpdateError) {
    throw new Error(
      `Failed to update recurring ${recurring.id}: ${recurringUpdateError.message}`,
    );
  }
...
async function handleSubscriptionDeleted(
  ...
  // Optimistic locking: only cancel if still active
  const { data: updated, error: updateError } = await admin
    .from("recurrings")
    .update({
      status: "cancelled",
  ...
  if (updateError && updateError.code !== "PGRST116") {
    throw new Error(
      `Failed to cancel recurring ${recurring.id}: ${updateError.message}`,
    );
  }
  ...
  // Fire-and-forget — don't block the webhook response
  void sendRecurringCancelEmail(admin, recurring.id).catch((err) =>
    console.error("Email error:", err),
  );
}
...
async function handleCheckoutSessionCompleted(
  ...
  if (!mosqueId || !plan) {
    console.error(
      "Webhook: checkout.session.completed missing mosque_id or plan metadata",
    );
    return;
  }
  ...
  if (updateError) {
    throw new Error(
      `Failed to update mosque ${mosqueId} plan to ${plan}: ${updateError.message}`,
    );
  }
}
...
async function sendRecurringPaymentConfirmation(
  ...
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
  const cancelUrl = rec?.cancel_token
    ? `${appUrl}/annuleren/${rec.cancel_token}`
    : undefined;
...
async function sendRecurringCancelEmail(
  ...
  const { data: recurring } = await admin
    .from("recurrings")
    .select(
      "amount, frequency, fund_id, donor_id, mosque_id, funds(name), donors(name, email), mosques(name, contact_email)",
    )
    .eq("id", recurringId)
    .single();
...
async function handleAccountUpdated(
  ...
  if (chargesEnabled) {
    ...
    if (updateError) {
      throw new Error(
        `Failed to mark mosque ${mosque.id} as Stripe connected: ${updateError.message}`,
      );
    }
    console.log(
      `[Stripe Connect] Mosque ${mosque.id} is now connected (charges enabled)`,
    );
  } else {
    ...
    if (updateError) {
      throw new Error(
        `Failed to clear Stripe connected status for mosque ${mosque.id}: ${updateError.message}`,
      );
    }
    console.warn(
