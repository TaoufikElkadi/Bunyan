...
export async function POST(request: Request) {
  try {
    ...
    const { success } = await rateLimit(`anbi-send:${ip}`, 10, 60_000);
    if (!success) {
    ...
    const { data: receipt, error: receiptError } = await admin
      .from("anbi_receipts")
      .select(
        "id, donor_id, year, total_amount, receipt_number, pdf_path, emailed_at, donors(name, email)",
      )
      .eq("id", receipt_id)
    ...
    if (receiptError || !receipt) {
      return NextResponse.json(
        { error: "Verklaring niet gevonden" },
        { status: 404 },
      );
    }

    const donor = receipt.donors as unknown as {
      name: string | null;
      email: string | null;
    } | null;
    ...
    if (downloadError || !pdfData) {
      ...
      return NextResponse.json(
        { error: "PDF kon niet worden opgehaald" },
        { status: 500 },
      );
    }
