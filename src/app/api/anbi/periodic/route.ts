...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Fout bij ophalen overeenkomsten' }, { status: 500 })
    }

    const formatted = (agreements ?? []).map((a) => {
      const donor = a.donors as unknown as { name: string | null; email: string | null; address: string | null } | null
...
export async function POST(request: Request) {
  try {
    ...
    const { donor_id, annual_amount, fund_id, start_date, end_date, notes } = body as {
      donor_id: string
