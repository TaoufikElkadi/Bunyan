...
export async function POST(request: Request) {
  try {
    ...
    if (!lines || !Array.isArray(lines) || lines.length === 0) {
      return NextResponse.json({ error: 'Geen donaties opgegeven' }, { status: 400 })
    }
    ...
    if (validLines.length === 0) {
      return NextResponse.json({ error: 'Vul minimaal één bedrag in' }, { status: 400 })
    }
    ...
    if (invalidFunds.length > 0) {
      return NextResponse.json({ error: 'Een of meer fondsen zijn ongeldig' }, { status: 400 })
    }

    // Build donation rows
    const createdAt = new Date(date).toISOString()
    const batchNote = notes?.trim()
      ? `collecte: ${notes.trim()}`
      : 'collecte'
    ...
    if (insertError) {
      ...
      return NextResponse.json({ error: 'Donaties aanmaken mislukt' }, { status: 500 })
    }
