...
export async function DELETE(request: Request) {
  try {
    ...
    const { success } = await rateLimit(`delete-mosque:${ip}`, 3, 60_000);
    if (!success) {
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Moskeenaam komt niet overeen" },
        { status: 400 },
      );
    }
    ...
    if (
      !mosque ||
      confirmName.trim().toLowerCase() !== mosque.name.trim().toLowerCase()
    ) {
      return NextResponse.json(
        { error: "Moskeenaam komt niet overeen" },
        { status: 400 },
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Moskee verwijderen mislukt" },
        { status: 500 },
      );
    }
