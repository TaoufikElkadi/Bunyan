...
export async function POST() {
  try {
    ...
    if (!profile || profile.role !== 'admin') {
      return NextResponse.json({ error: 'Alleen beheerders kunnen Stripe verbinden' }, { status: 403 })
    }
    ...
    // If already fully connected, return status
    if (mosque.stripe_connected_at) {
      return NextResponse.json({
    ...
    return NextResponse.json({ error: 'Stripe verbinding mislukt' }, { status: 500 })
  }
}
