...
export default async function CancelPage({ params }: Props) {
  ...
  const fundName = (recurring.fund as unknown as { name: string })?.name || 'Onbekend fonds'
  const mosqueName = (recurring.mosque as unknown as { name: string })?.name || 'Onbekende moskee'

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-lg px-4 py-12">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold">{mosqueName}</h1>
        </div>
        <CancelForm
