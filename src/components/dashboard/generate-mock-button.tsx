...
export function GenerateMockButton() {
  ...
  async function handleGenerate() {
    ...
    if (res.ok) {
      setResult(
        `${data.created.donors} donateurs, ${data.created.donations} donaties, ${data.created.recurrings} terugkerend`,
      );
      revalidateAll();
    } else {
  ...
  return (
    <div className="rounded-2xl border border-dashed border-[#e3dfd5] bg-[#fafaf8] p-4">
      ...
      {result && <p className="mt-2 text-[11px] text-[#8a8478]">{result}</p>}
    </div>
  );
}
