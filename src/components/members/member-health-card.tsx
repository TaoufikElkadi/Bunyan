...
export function MemberHealthCard() {
    ...
    return (
      <div className="rounded-2xl bg-white p-5 shadow-[0_1px_3px_rgba(38,27,7,0.04),0_1px_2px_rgba(38,27,7,0.02)]">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[13px] font-semibold text-[#261b07]">Leden gezondheid</h3>
        </div>
        <div className="flex flex-col items-center py-4">
          <Users className="h-5 w-5 text-[#b5b0a5] mb-2" strokeWidth={1.5} />
          <p className="text-[11px] text-[#a09888] text-center">
            Nog geen leden. Zodra er donaties binnenkomen, verschijnen hier inzichten.
          </p>
        </div>
      </div>
    )
  ...
  const retentionRate = totalIdentified > 0
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-[0_1px_3px_rgba(38,27,7,0.04),0_1px_2px_rgba(38,27,7,0.02)]">
      ...
      <div className="space-y-2.5">
        {bars.map((bar) => (
          <div key={bar.label} className="flex items-center gap-3">
            <div className="w-[60px] text-[11px] text-[#8a8478] shrink-0">{bar.label}</div>
      ...
      {stats.high_churn_risk > 0 && (
        ...
        >
          <AlertTriangle className="h-3.5 w-3.5 text-[#8a6d00] shrink-0" strokeWidth={1.5} />
          <span className="text-[11px] text-[#8a6d00]">
            <span className="font-semibold">{stats.high_churn_risk}</span> leden met hoog verlooprisico
          </span>
        </Link>
      )}
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-[#e3dfd5]/60">
        <div className="text-[11px] text-[#a09888]">
          Retentie <span className="font-semibold text-[#261b07]">{retentionRate}%</span>
        </div>
        <div className="text-[11px] text-[#a09888]">
          Gem. gift <span className="font-semibold text-[#261b07]">{formatMoney(stats.avg_donation)}</span>
        </div>
      </div>
    </div>
  )
