...
export function SegmentCards() {
  const { data, isLoading } = useSWR<{ segments: Segment[] }>('/api/members/segments', fetcher, {
      ...
      <div className="rounded-xl border border-[#e3dfd5] bg-white p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-16 rounded-lg bg-[#f3f1ec] animate-pulse" />
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div className="rounded-xl border border-[#e3dfd5] bg-white p-6">
      <h3 className="text-[15px] font-semibold text-[#261b07] mb-1">Segmenten</h3>
      <p className="text-[12px] text-[#a09888] mb-5">Voorgedefinieerde groepen</p>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[12px] text-[#8a8478] truncate">{segment.label}</p>
                <p className="text-[18px] font-bold text-[#261b07] leading-tight tabular-nums">{segment.count}</p>
              </div>
            </div>
          )
        })}
