...
const NAME_ICON_MAP: [RegExp, LucideIcon][] = [
  [/zakat al-fitr|zakat.?fitr/i, Star],
  [/zakat/i, HandHeart],
...
export function FundCards({ funds, totals, role }: FundCardsProps) {
  ...
  async function handleArchive() {
    ...
    try {
      const res = await fetch(`/api/funds/${archiveTarget.id}`, {
        method: "DELETE",
      });
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {funds.map((fund, index) => {
          ...
          const goalProgress = goalAmount
            ? Math.min(100, Math.round((total / goalAmount) * 100))
            : null;
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div
                    className={`flex h-10 w-10 items-center justify-center rounded-xl ${colorSet.bg}`}
                  >
                    <Icon
                      className={`h-[18px] w-[18px] ${colorSet.text}`}
                      strokeWidth={1.5}
                    />
                  </div>
                  <div>
                    <h4 className="text-[14px] font-semibold text-[#261b07] leading-tight">
                      {fund.name}
                    </h4>
                    <span
              ...
              {/* Description */}
              {fund.description && (
                <p className="text-[12px] text-[#8a8478] mb-4 line-clamp-2">
                  {fund.description}
                </p>
              )}
                  ...
                  <>
                    ...
                    {fund.goal_deadline && (
                      <div className="flex items-center gap-1 mt-1.5">
                        <Calendar
                          className="h-3 w-3 text-[#b5b0a5]"
                          strokeWidth={1.5}
                        />
                        <span className="text-[10px] text-[#b5b0a5]">
                          Deadline:{" "}
                          {new Date(fund.goal_deadline).toLocaleDateString(
                            "nl-NL",
                            {
                              day: "numeric",
                              month: "short",
                  ...
                  <div className="h-1.5 w-full rounded-full bg-[#f3f1ec] overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor:
                          FUND_ICON_COLORS[index % FUND_ICON_COLORS.length].hex,
                      }}
                    />
                  </div>
                )}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Weet u zeker dat u &ldquo;{archiveTarget?.name}&rdquo; wilt
              archiveren? Het fonds wordt verborgen op de donatiepagina, maar
              bestaande donaties blijven bewaard.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
