...
export function donationConfirmationEmail(params: DonationConfirmationParams): string {
  const {
    mosqueName,
    donorName,
    amount,
              ...
              <tr>
                <td style="padding: 12px 0 0 0;">
                  <p style="margin: 0; font-size: 14px; color: #3f3f46; line-height: 1.6;">
                    Dit is een <strong>${frequencyLabel[frequency || 'monthly'] || frequency}</strong> doorlopende donatie.
                  </p>
                  ${cancelUrl ? `
  ...
  const body = `
              ...
              <p style="margin: 0 0 24px 0; font-size: 16px; color: #3f3f46; line-height: 1.6;">
                Bedankt voor uw donatie van <strong>${formattedAmount}</strong> aan ${fundName} bij ${mosqueName}.
              </p>
  ...
  return emailLayout({ title: `Donatiebevestiging - ${mosqueName}`, body })
}
